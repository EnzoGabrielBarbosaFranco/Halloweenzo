...
onMounted(() => {
  window.onscroll = () => {
    if (window.scrollY === 0) {
      cabecalho.value.classList.remove('bg-primary-200')
    } else {
      cabecalho.value.classList.add('bg-primary-200')
    }
  }
})
</script>
...
<template>
  <nav ref="cabecalho" class="fixed top-0 left-0 w-full bg-primary-200 z-20 transition duration-200">
    <div class="container flex items-center justify-between h-20">
      <div class="relative z-30 flex items-center gap-2 mb-2 lg:mb-0">
        <h1 class="text-white font-bold text-lg lg:text-2xl">Hallow<span class="text-secondary">ee</span>nzo</h1>
      ...
      <div class="
        ...
        <a @click="showMenu = false" class="text-white font-medium" href="#category">Categorias</a>
        <a @click="showMenu = false" class="text-white font-medium" href="#about">Sobre</a>
        <a @click="showMenu = false" class="text-white font-medium" href="#trick-or-treat">Doçes ou Travessuras</a>
        <button
          class="hidden md:block px-8 py-3 rounded-full text-white font-medium tracking-wider uppercase bg-secondary w-full lg:w-auto">Doações</button>
      </div>
    </div>
  </nav>
</template>